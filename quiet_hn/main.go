...
import (
	...
	"github.com/jeremy-miller/gophercises/quiet_hn/hn"
)
...
func handler(numStories int, tpl *template.Template) http.HandlerFunc {
	// create cache outside of HandlerFunc so it's not recreated on every request
	sc := storyCache{
		numStories: 30,
	...
	go func() {
		...
		for {
			...
			sc.mutex.Unlock()
			<-ticker.C // ensure the story cache gets populated on start
		}
	}()
