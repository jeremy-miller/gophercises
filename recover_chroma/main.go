...
func makeLinks(stack string) string {
	...
	for li, line := range lines {
		...
		for i := len(file) + 2; i < len(line); i++ { // +1 for tab and +1 for ":" in line
...
func sourceCodeHandler(w http.ResponseWriter, r *http.Request) {
	...
	formatter := html.New(html.Standalone(true), html.TabWidth(2), html.WithLineNumbers(true), html.HighlightLines(lines))
